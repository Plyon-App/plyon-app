...
export const DataProvider: React.FC<{ children: ReactNode; initialData?: any; readOnlyMode?: boolean }> = ({ children, initialData: propData, readOnlyMode = false }) => {
  ...
  const importJsonDataV2 = async (json: string) => {
    if (!user || !db) {
    ...
    for (const match of matches) {
      await setDoc(doc(db, "users", user.uid, "football_activities", match.id), match);
    }
    
    const tournaments = data.tournaments || [];
    for (const tournament of tournaments) {
      await setDoc(doc(db, "users", user.uid, "football_tournaments", tournament.id), tournament);
    }
  ...
  return (
    <DataContext.Provider value={{
      ...
      importJsonData, importJsonDataV2, importCsvData, importMatchesFromAI, checkAILimit, aiUsageCount, AI_MONTHLY_LIMIT,
