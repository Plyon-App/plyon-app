...
interface TennisHistoricalAnalysisProps {
  matches: Match[];
}
  ...
  const statsByYearAndMonth = React.useMemo(() => {
    ...
    matches.forEach(match => {
      ...
      if (!data[year][capitalizedMonth]) {
        data[year][capitalizedMonth] = { wins: 0, losses: 0, setsWon: 0, setsLost: 0, gamesWon: 0, gamesLost: 0, matchCount: 0, matches: [] };
      }
      
      const stats = data[year][capitalizedMonth];
      stats.matchCount++;
      stats.setsWon += match.myGoals || 0;
      stats.setsLost += match.opponentGoals || 0;
      stats.gamesWon += match.myAssists || 0;
      stats.gamesLost += match.opponentAssists || 0;
            ...
            {expandedYear === year && (
              <div style={styles.monthContainer} className="fade-in-down">
                {Object.keys(statsByYearAndMonth[year]).sort((a, b) => monthOrder.indexOf(a) - monthOrder.indexOf(b)).reverse().map(month => {
                  ...
                  const efectividad = stats.matchCount > 0 ? (stats.wins / stats.matchCount) * 100 : 0;
                          ...
                          {isExpanded && (
                              <div className="fade-in-down">
                                  <div style={{...styles.statsGrid, borderRadius: stats.matches.length > 0 ? 0 : `0 0 ${theme.borderRadius.medium} ${theme.borderRadius.medium}` }}>
                                      ...
                                      <div style={styles.statItem}><span style={styles.statIcon}>🎾</span><span style={styles.statValue}>{stats.setsWon}</span><span style={styles.statLabel}>Sets G.</span></div>
                                      <div style={styles.statItem}><span style={styles.statIcon}>🔻</span><span style={styles.statValue}>{stats.setsLost}</span><span style={styles.statLabel}>Sets P.</span></div>
                                      <div style={styles.statItem}><span style={styles.statIcon}>🔥</span><span style={styles.statValue}>{stats.gamesWon}</span><span style={styles.statLabel}>Games G.</span></div>
                                      <div style={styles.statItem}><span style={styles.statIcon}>📉</span><span style={styles.statValue}>{stats.gamesLost}</span><span style={styles.statLabel}>Games P.</span></div>
                                  </div>
                                          ...
                                          {stats.matches.map((match, index) => (
                                              <div key={match.id} style={ index === stats.matches.length - 1 ? {...styles.matchRow, ...styles.lastMatchRow} : styles.matchRow}>
                                                  ...
                                                  <div style={styles.matchStats}>
                                                      <span style={styles.matchStatItem}>🎾 {match.myGoals}</span>
...
export default TennisHistoricalAnalysis;
