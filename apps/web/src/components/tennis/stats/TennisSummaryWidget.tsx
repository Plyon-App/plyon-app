...
const TennisSummaryWidget: React.FC<TennisSummaryWidgetProps> = ({ matches }) => {
  ...
  const stats = useMemo(() => {
    ...
    filteredMatches.forEach(match => {
      if (match.tennisScore?.sets) {
        match.tennisScore.sets.forEach(set => {
          if (set.myGames > set.opponentGames) setsWon++;
          else if (set.opponentGames > set.myGames) setsLost++;
  ...
  return (
    <Card title={<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}><span>Resumen</span><YearFilter years={availableYears} selectedYear={selectedYear} onChange={setSelectedYear} /></div>}>
